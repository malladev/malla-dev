...
export function Contact() {
    return (
        <Layout>
            <div id="contact" className="my-20 mx-auto max-w-5xl">
                <ContactBackground className="rounded-[22px] p-4 sm:p-10 bg-gray-900">
                    <p className="text-2xl sm:text-3xl mt-4 mb-2 font-bold text-neutral-200">
                        Contacts
                    </p>
                    ...
                    <div className="flex items-center gap-2 flex-wrap mt-3">
                        {contacts.map((contact) => (
                            <a key={contact.id} href={contact.url} className={`${contact.color} py-2 px-6 w-fit rounded text-sm flex items-center justify-center gap-1`}>
                                <img src={contact.icone} alt={contact.name} className="white-icon w-4 h-4" />
                                {contact.name}
                            </a>
                        ))}
                        <a href="mailto:konelamine1122@gmail.com" className="py-2 px-6 w-fit rounded text-sm flex items-center justify-center gap-1 bg-slate-100">
